...
@section('content')
	...
	@php if($topic->video_thumb){$poster = asset('lessions/'. $topic->video_thumb); }else{$poster =''; } @endphp
	<!-- <div class="plan mt-3" data-aos="fade-right">
		<div class="container">
			<iframe src="{{asset('lessions') . '/' . $topic->fullvideo}}" title="Video player"  frameborder="0" controls="0" autoplay="0" allowfullscreen sandbox></iframe>
		</div>
	</div> -->
	<div class="container text-center mt-3" style="position: relative;">
		...
		>
			<source src="{{asset('lessions/'. $topic->fullvideo)}}" type="video/mp4" />
		...
		<div class="text-prt">
			...
			<div class="row">
				...
				<div class="col-md-5 text-right">
					...
					@if(isset($topic->pdf) && $topic->pdf!='')
					@if($topic->isFree==1)
					@if($userSubscription==1)
					<a href="{{asset('lessions') . '/' . $topic->pdf}}" class="buttonDownload" target="_blank"><img src="{{asset('front/assets/img/download.svg')}}"> Download</a>
					@else
					<a href="{{route('topicDetails',[$topic->id,'error'=>'subscription'])}}" class="buttonDownload"><img src="{{asset('front/assets/img/download.svg')}}"> Download</a>
					@endif
					@else
					<a href="{{asset('lessions') . '/' . $topic->pdf}}" class="buttonDownload" target="_blank"><img src="{{asset('front/assets/img/download.svg')}}"> Download</a>
					@endif
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<form action="{{route('rateByUser')}}" method="post"> 
								...
								<input type="hidden" name="courseId" value="{{$topic->courseId}}">
								<input type="hidden" name="lessionId" value="{{$topic->lessionId}}">
														...
														<div class="mail_sec ">
															<div class="btn-group btn-group-toggle" data-toggle="buttons">
																<label class="btn btn-default btn-lg toggle-checkbox primary active">
																	<input type="radio" id="ratingType" name="ratingType" onclick="Check();" style="display: none;" value="Sad" checked />
																	<img src="{{asset('front/assets/img/sad.svg')}}">
																</label>
																<label class="btn btn-default btn-lg toggle-checkbox primary">
																	<input type="radio" id="ratingType" name="ratingType" onclick="Check();" style="display: none;" value="Sceptic" />
																	<img src="{{asset('front/assets/img/sceptic.svg')}}">
																</label>
																<label class="btn btn-default btn-lg toggle-checkbox primary">
																	<input type="radio" id="ratingType" name="ratingType" onclick="Check();" style="display: none;" value="Happy" />
