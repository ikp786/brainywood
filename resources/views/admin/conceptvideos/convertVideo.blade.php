		...
		<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default">
					<div class="panel-heading">All Concept Videos Converted Video</div>
					<div class="panel-body">
						...
						<table class="table table-bordered" id="courses">
							...
							@foreach($data as $single)
								<tr>
									<td>{{$i}}</td>
									<td>{{$single->conceptvideo->name}}</td>
									...
									<td>
										...
										<a href="{{asset('upload/conceptvideos') . '/' . $single->low_video}}" target="_blank">View</a>
										@endif
										@if($single->med_status == 1)
										<a href="{{asset('upload/conceptvideos') . '/' . $single->med_video}}" target="_blank">View</a>
										@endif
										@if($single->high_status == 1)
										<a href="{{asset('upload/conceptvideos') . '/' . $single->high_video}}" target="_blank">View</a>
										@endif
										<a href="{{route('admin.approveVideoConceptVideo',['id'=> $single->id])}}">Approve</a>
										<!-- <a href="{{route('admin.deleteConvertedConceptVideo',['id'=> $single->id])}}" onclick="return confirm('Are you sure you want to delete this?');">Delete</a> -->
									</td>
								</tr>
								  <?php $i++; ?>
							@endforeach
