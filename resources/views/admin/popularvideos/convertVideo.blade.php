		...
		<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default">
					<div class="panel-heading">All Popular Videos Converted Video</div>
					<div class="panel-body">
						...
						<table class="table table-bordered" id="courses">
							...
							@foreach($data as $single)
								<tr>
									<td>{{$i}}</td>
									<td>{{$single->popularvideo->name}}</td>
									...
									<td>
										...
										<a href="{{asset('upload/popularvideos') . '/' . $single->low_video}}" target="_blank">View</a>
										@endif
										@if($single->med_status == 1)
										<a href="{{asset('upload/popularvideos') . '/' . $single->med_video}}" target="_blank">View</a>
										@endif
										@if($single->high_status == 1)
										<a href="{{asset('upload/popularvideos') . '/' . $single->high_video}}" target="_blank">View</a>
										@endif
										<a href="{{route('admin.approveVideoPopularVideo',['id'=> $single->id])}}">Approve</a>
